...
<template>
  ...
  >
    <UContainer class="py-3 md:py-4 flex items-center justify-between">
      <div class="flex items-center gap-8 flex-shrink-0">
        <button class="lg:hidden h-fit" @click="isBurgerOpen = true">
          <img src="~/assets/icons/menu.svg" alt="menu" class="size-6" />
        </button>
        <NuxtLinkLocale to="/" class="h-10 lg:h-12 flex-shrink-0">
          <img
      ...
      <div class="flex gap-6 group max-lg:hidden items-center flex-shrink">
        <NuxtLinkLocale
          ...
          class="text-base font-medium text-white group-hover:text-white-500 hover:!text-white transition-all duration-300 whitespace-nowrap text-center"
        >
          {{ item.title }}
        </NuxtLinkLocale>
      </div>
      ...
      <div class="flex items-center flex-shrink-0">
        ...
        <AppLang class="md:!hidden ml-auto" />
      </div>
    </UContainer>
  </header>
</template>
